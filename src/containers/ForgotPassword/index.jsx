...
import { forgetPassword, resetPassword, checkPasswordDate } from "../../actions";
import { Link } from "react-router-dom";


export default function ForgetPassword({ forgetPassword, resetPassword, resetPasswordSucsses, forgetPasswordSucsses, checkPasswordDate, checkPasswordDateSucsses }) {
  ...
  React.useEffect(() => {
    if (forgetPasswordSucsses) if (isSubmit) {
      if (forgetPasswordSucsses.status == 200) {
        ...
        NotificationModel("bg-danger", "wait for 30 minuetes after getting password")
      } setIsSubmit(false)
    }
        ...
        <div className="card-body">
          <Form noValidate validated={isSubmit} onSubmit={submitForgotPassword}>
            <Form.Group>
              ...
              <Input required onChange={e => setmobile(e.target.value)} className="form-control-alternative" type="tel" placeholder="Enter phone number" pattern="^[789]\d{9,9}$" />
              {validateMsgValid}
              {validateMsgInvalid}
              <br />
  ...
  function submitForgotPassword(e) {
    ...
    if (e.currentTarget.checkValidity()) {
      if (Password == ConfirmPassword) {
        forgetPassword({ 'email': email, 'mobile': mobile, "userId": localStorage.getItem("userid") }, localStorage.getItem("token"));
