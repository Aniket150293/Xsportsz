...
router.post('/forgetPassword', function (req, res, next) {
  ...
  connection.query('SELECT TIMESTAMPDIFF(SECOND ,pudate,NOW()) as tm  from user_details where email = ? and mobile = ?',
    // [req.body.email],
    ...
    function (err, rows) {
      ...
      if (err) {
        res.send({ status: 500, data: {}, msg: "Please provide valid Email and Password" });
        console.log(err);
      }
      else {
          ...
          if ((rs.tm >= 1800) == true) {
            ...
            transporter.sendMail(mailOptions, function (error, info) {
              if (error) {
                ...
                connection.query('UPDATE user_details SET password = ? , pudate=now() where email = ?',
                ...
                res.send({ status: 200, data: rows, msg: "Password get success" });
