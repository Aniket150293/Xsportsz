  ...
  jwt.verify(req.headers['authorization'], toString(req.body.userid), function (err, data) {
    if (err) {
      ...
      if (req.body.months != "") {
        ...
        var y = e.getFullYear(), m = e.getMonth() + 1 - req.body.months
        if (m <= 0) {
          m = m + 12
          y = y - 1
        }
        ...
        var a = connection.query("SELECT specialization_master.name as sname, user_sport_mapping.user_id as id,user_details.first_name,user_details.email,user_details.mobile ,user_sport_mapping.years_age,user_sport_mapping.months_age,sports_master.name FROM user_sport_mapping INNER JOIN specialization_master ON specialization_master.id=user_sport_mapping.sport_id INNER JOIN user_details on user_details.id=user_sport_mapping.user_id INNER JOIN sports_master on sports_master.id=user_sport_mapping.sport_id and user_sport_mapping.created_date between '" + y1 + "-" + getMonth1 + "-" + getDate1 + "' and '" + getFullYear1 + "-" + m1 + "-" + getDate1 + "'",
          function (err, rows) {
            if (err) {
              res.send({ status: 500, data: {} });
            } else {
              if (typeof rows != 'undefined' && rows != '' && rows != null) {
                res.send({ status: 200, data: rows, msg: 'Listsssssssssssssssssssss Fetched Successfully' });
              } else {
                res.send({ status: 500, data: {} });
              }
            }
